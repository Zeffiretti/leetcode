...
class Solution {
 public:
  ...
  void dfs(TreeNode* root, unordered_map<TreeNode*, TreeNode*>* max_sum_root) {
    if (!root) {
      return;
    }
    ...
    if ((*max_sum_root)[root->left] && (*max_sum_root)[root->right]) {
      (*max_sum_root)[root]->val += max(max((*max_sum_root)[root->left]->val, (*max_sum_root)[root->right]->val), 0);
    ...
    if ((*max_sum_root)[root->left] && (*max_sum_root)[root->left]->val > 0) {
      tmp_sum += (*max_sum_root)[root->left]->val;
    }
    if ((*max_sum_root)[root->right] && (*max_sum_root)[root->right]->val > 0) {
