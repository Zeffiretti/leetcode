...
class Solution {
 public:
  vector<int> dailyTemperatures(vector<int>& temperatures) {
    ...
    vector<int> nge(n,
                    0);  // initially all 0, stores distance between their next greater element and current temperature
    stack<int> st{};
    ...
    for (int i = n - 1; i >= 0; --i) {
      ...
      if (!st.empty()) nge[i] = st.top() - i;  // distance between next greater and current
